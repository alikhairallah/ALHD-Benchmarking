...
LOGS_FOLDER         = f"../../../logs/benchmarking/{EXPERIMENT_TYPE}/{DATA_SIZE}/ext" #update log folder path
SEED                = 42
...
MODELS = {
    "Falcon-Arabic-7B-Instruct": {
        "hf_id": "tiiuae/Falcon3-7B-Instruct",  # Falcon 3 Arabic-instruct variant
        "family": "falcon",
        "chat_template": "auto",
        "quantize_8bit": False,
    ...
    "Command-R7B-Arabic": {
        "hf_id": "CohereLabs/c4ai-command-r7b-arabic-02-2025",  # Cohere Labs Arabic command model
        "family": "command",
        "chat_template": "auto",
        "quantize_8bit": False,
        "force_no_cache": False,
    ...
    "Gemma-3-12b-it": {
        "hf_id": "google/gemma-3-12b-it",  # Google Gemma 3 instruction-tuned, multilingual incl. Arabic
        "family": "gemma",
        "chat_template": "auto",
        "quantize_8bit": False,
        "force_no_cache": False,
...
MODEL_BS = {
    "Falcon-Arabic-7B-Instruct":   {"zs": 32, "fs": 16},
    "Command-R7B-Arabic":     {"zs": 32, "fs": 16},
    "Gemma-3-9B-Instruct":           {"zs": 32, "fs": 16},
}
...
def _load_llm_for_textgen(cfg_entry):
    ...
    v_model = getattr(mdl.config, "vocab_size", mdl.get_input_embeddings().weight.shape[0])
    v_tok   = getattr(tok, "vocab_size", len(tok))
    if v_tok == v_model:
        try:
            mdl.resize_token_embeddings(v_tok)
        except Exception:
            pass
    else:
        logging.info(f"Skip resize_token_embeddings: tokenizer={v_tok}, model={v_model}")
    mdl.eval()
